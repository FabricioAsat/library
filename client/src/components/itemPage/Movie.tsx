...
export const Movie = ({
  ...
  const [item, setItem] = useState<IMovie | null>(null);
  const [infoShown, setInfoShown] = useState<IInfoShown>();

  useEffect(() => {
    setIsLoading(true);
    const fetchItem = async () => {
      const res = await getItem(id, "movie");
      if (!res.status || !res.data) {
        toast.error(res.message);
        setIsLoading(false);
        return;
      }
      setItem((res.data as IMovie) || null);
      setIsLoading(false);
    };
          ...
          <div className="flex flex-col w-full md:pt-10 gap-y-5">
            <div className="flex items-center gap-x-3">
              ...
              <div className="flex flex-col">
                ...
                <h3 className="text-lg italic">{item.Director || "Unknown"}</h3>
              </div>
            </div>
